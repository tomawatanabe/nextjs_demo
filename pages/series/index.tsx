...
const ResultOfSeries = ({products, selected}: {products: Stock[]; selected: string}) => {
...
export const getServerSideProps = async (context: any) => {
    const seriesName = context.query.seriesName
    console.log(seriesName);

    
    let { data: products, error } = await supabase.from("stocks").select(`*,items(*)`);

    const result = products?.filter((stock: Stock) => stock.items.series === seriesName);
    if(error) {
        throw Error;
    }
